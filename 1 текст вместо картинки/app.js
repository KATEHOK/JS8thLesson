...
function handleClick(clickedButtonEvent) {
    ...
    let textOfButton = card.button.innerText;

    if (textOfButton == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (textOfButton == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let randomText = 'Текст - описание товара';
